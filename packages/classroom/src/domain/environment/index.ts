...
export class Environment {
  ...
  static readonly MESSAGING = Object.freeze({
    NAME: env.get('MESSAGING_NAME').required().asString(),
    KAFKA: {
      KAFKA_BROKER: env.get('KAFKA_BROKER').required().asString(),
      KAFKA_USER: env.get('KAFKA_USER').asString(),
      KAFKA_PASS: env.get('KAFKA_PASS').asString(),
      KAFKA_SSL: env.get('KAFKA_SSL').required().asBool(),
